...
**Plan 未完成时，禁止开始写代码。** spec-graph dev 会自动拒绝执行。

```
用户需求 → Agent 自动决策:
  ├─ 新功能 → [必须] spec-graph dispatch → 生产 PRD/设计/Epics/Stories
  │           循环直到 gate 通过，然后才能 spec-graph dev
  └─ 查询/澄清 → 直接回答
```

**Agent 自动判断，不需要用户说"先规划"。PM → Architect → Developer 按序生产。**

...
exports.renderTerminalDashboard = renderTerminalDashboard;
exports.renderHtmlDashboard = renderHtmlDashboard;
/**
 ...
 */
function renderHtmlDashboard(data) {
    const artPct = data.stats.total_artifacts > 0
        ? Math.round((data.stats.completed_artifacts / data.stats.total_artifacts) * 100)
        : 0;
    const chkPct = data.stats.total_checks > 0
...
<div class="section">
  ...
  <div class="artifact-grid">
    ${Object.entries(data.artifacts).map(([id, info]) => `<div class="artifact"><span class="dot ${info.status}"></span>${id} <span style="color:#8b949e;font-size:0.75rem">${info.kind || ""}</span></div>`).join("")}
  </div>
</div>

<div class="section">
  <h2>Gates</h2>
  ${data.gates.map(g => `<div class="gate ${g.passed ? "passed" : "blocked"}">

...
async function runCompose(projectRoot, rawProfile, changeType = "feature") {
    const warnings = [];
    const errors = [];
    ...
    if (!profile.meta?.source?.reviewed_at) {
        warnings.push("composing from an unreviewed profile — run `spec-graph profile review` to freeze it after human check");
    }
    ...
    if (!intentPack) {
        errors.push(`No intent pack matched for change type: ${changeType}, falling back to feature`);
    ...
    const activePacks = domainPacks.sort((a, b) => (b.priority || 0) - (a.priority || 0));
    // Step 4: Merge artifacts
    const { artifacts: mergedArtifacts, warnings: artifactWarnings } = mergeArtifacts(activePacks);
    warnings.push(...artifactWarnings);
    // Step 5: Merge actions
    const mergedActions = mergeActions(activePacks);
    // Step 6: Merge checks
    const { checks: mergedChecks, warnings: checkWarnings } = mergeChecks(activePacks);
    ...
    const { tracks, warnings: trackWarnings } = assembleTracks(activePacks, mergedArtifacts);
    ...
    const layerInjectionWarnings = injectRequiredLayerChecks(gates, acceptanceLayers);
    ...
    const conflictErrors = detectConflicts(mergedArtifacts, mergedChecks, gates, tracks);
    ...
    const { agentBindings, warnings: bindingWarnings } = mergeAgentBindings(activePacks);
    ...
    const graph = {
        ...
        meta: {
            ...
            profile_hash: node_crypto_1.default
                .createHash("sha256")
...
async function loadPacks(projectRoot) {
    const packMap = new Map();
    const packDirs = [
        node_path_1.default.resolve(__dirname, "../../../packs"),
        node_path_1.default.join(projectRoot, "packs"),
    ];
 ...
 */
async function loadProjectConfig(projectRoot, warnings) {
    const configPath = node_path_1.default.join(projectRoot, ".spec-graph", "config.yaml");
    let raw;
    try {
        raw = await (0, yaml_1.tryReadYaml)(configPath);
    }
    catch {
        return undefined;
    }
 ...
 */
async function applyPackOverrides(projectRoot, packs, warnings) {
    const overridesPath = node_path_1.default.join(projectRoot, ".spec-graph", "pack-overrides.yaml");
    let overrides;
    try {
        overrides = await (0, yaml_1.tryReadYaml)(overridesPath);
    }
    catch {
    ...
    for (const [packName, override] of Object.entries(overrides.overrides)) {
        ...
        if (!pack) {
            warnings.push(`pack-overrides.yaml: pack '${packName}' not found, skipping`);
            continue;
        }
        // Apply check overrides
        if (override.checks && pack.provides?.checks) {
            for (const [checkId, checkOverride] of Object.entries(override.checks)) {
                ...
                if (check) {
                    ...
                    if (checkOverride.tier)
                        check.tier = checkOverride.tier;
                }
                else {
                    // Add new check
        ...
        if (override.gates && pack.provides?.gates) {
            for (const [gateId, gateOverride] of Object.entries(override.gates)) {
                const gate = pack.provides.gates.find((g) => g.id === gateId);
                if (gate) {
                    ...
                    if (gateOverride.remove_checks) {
                        gate.require_checks = (gate.require_checks || []).filter((c) => !gateOverride.remove_checks.includes(c));
                    }
        ...
        if (override.artifacts && pack.provides?.artifacts) {
            for (const [artifactId, artifactOverride] of Object.entries(override.artifacts)) {
                const artifact = pack.provides.artifacts.find((a) => a.id === artifactId);
                if (artifact) {
...
async function loadPacksFromDir(packsDir) {
    const packs = [];
    try {
        const entries = await promises_1.default.readdir(packsDir, { withFileTypes: true });
        for (const entry of entries) {
            if (!entry.isDirectory())
                continue;
            const packYaml = node_path_1.default.join(packsDir, entry.name, "pack.yaml");
            try {
                const content = await promises_1.default.readFile(packYaml, "utf-8");
...
function normalizePack(parsed, dirname) {
...
function inferPackKind(parsed, dirname) {
    if (parsed.applies_when_change)
        return "change-intent";
    if (dirname.includes("feature") || dirname.includes("bugfix"))
        return "change-intent";
    return "domain";
}
function createFoundationFallbackPack() {
    return {
        name: "foundation",
        version: "1.0.0",
        kind: "domain",
...
function createFeatureFallbackPack() {
    return {
        name: "feature",
        version: "1.0.0",
        kind: "change-intent",
...
function matchDomainPacks(packs, profile) {
    return packs.filter((p) => p.kind === "domain" && evaluateAppliesWhen(p.applies_when, profile));
}
...
function applyOverrides(profile, warnings) {
    ...
    for (const [dim, value] of Object.entries(overrides)) {
        ...
        if (prior && prior.value !== value) {
            warnings.push(`override: ${dim}=${value} (was ${prior.value} from ${prior.source})`);
        }
        facts[dim] = {
            value,
            confidence: "high",
...
function matchIntentPack(packs, changeType) {
    return (packs.find((p) => {
        if (p.kind !== "change-intent")
            return false;
...
function evaluateAppliesWhen(condition, profile) {
    if (condition === "always")
        return true;
    if (!condition)
        return false;
    for (const [key, expected] of Object.entries(condition)) {
        if (key === "$or") {
            return expected.some((subCond) => evaluateAppliesWhen(subCond, profile));
        }
        if (key === "$and") {
            return expected.every((subCond) => evaluateAppliesWhen(subCond, profile));
        }
        const actual = profile.facts[key]?.value;
        ...
        for (const artifact of pack.provides?.artifacts || []) {
            ...
            if (!existing) {
                ...
                if (existing.schema_ref !== artifact.schema_ref &&
                    ...
                    warnings.push(`Artifact schema conflict for ${artifact.id}: ${existing.provided_by} vs ${pack.name}`);
                }
            }
        }
    }
...
function assembleGates(packs) {
    ...
    for (const pack of packs) {
        ...
        for (const [gateId, patch] of Object.entries(patches)) {
            ...
            if (!gate) {
                warnings.push(`Gate patch target '${gateId}' not found, provided by pack '${pack.name}'`);
                continue;
            }
...
function assembleTracks(packs, artifacts) {
    const warnings = [];
    const tracks = [];
    for (const pack of packs) {
    ...
    for (const [scope, count] of scopeCounts.entries()) {
        if (count > 1) {
            warnings.push(`Multiple tracks share scope '${scope}' - this may cause conflicts`);
        }
    }
 ...
 */
function detectSharedContractOpportunities(tracks) {
    const warnings = [];
    ...
    for (const track of tracks) {
        for (const c of track.consumes || []) {
            if (!c.startsWith("contract/"))
                continue;
            if (produced.has(c))
                continue; // already has a producer — not an opportunity
            if (!consumers.has(c))
                consumers.set(c, []);
    ...
    for (const [contractId, consumerList] of consumers.entries()) {
        if (consumerList.length >= 2) {
            warnings.push(`shared-contract opportunity: '${contractId}' consumed by ${consumerList.length} tracks (${consumerList.join(", ")}) but produced by none — consider introducing a contract/shared-lib (single producer, multi-consumer); review before wiring to avoid premature abstraction`);
        }
    }
    return warnings;
}
function selectPipelineSkeleton(intentPack, allPacks) {
...
function mergeAcceptanceLayers(packs) {
    const layers = {
    ...
    for (const pack of packs) {
        ...
        for (const [layerName, layerDef] of Object.entries(packLayers)) {
            ...
            layers[layerName].required ||= layerDef.required || false;
 ...
 */
function injectRequiredLayerChecks(gates, acceptanceLayers) {
    const warnings = [];
    ...
    for (const [layerName, def] of Object.entries(acceptanceLayers)) {
        if (def.required && def.checks.length === 0) {
            warnings.push(`acceptance layer '${layerName}' is required but has no checks — cannot enforce`);
            continue;
        }
        if (def.required) {
...
function deriveTraces(artifacts, gates, tracks) {
    const traces = [];
...
function detectConflicts(artifacts, checks, gates, tracks) {
    const errors = [];
    // Gate references non-existent artifact
    for (const gate of gates) {
        for (const artifactId of gate.require_artifacts || []) {
            if (!artifacts.some((a) => a.id === artifactId)) {
                errors.push(`Gate '${gate.id}' requires artifact '${artifactId}' but no pack provides it`);
            }
        }
        for (const checkId of gate.require_checks || []) {
            if (!checks.some((c) => c.id === checkId)) {
                errors.push(`Gate '${gate.id}' requires check '${checkId}' but no pack provides it`);
            }
        }
    }
    return errors;
    ...
    for (const pack of packs) {
        for (const agent of pack.provides?.agents || []) {
            ...
            if (!existing) {
                ...
                warnings.push(`agent '${agent.id}' overridden by higher-priority pack '${pack.name}' (was '${existing.pack}')`);
                agentMap.set(agent.id, {
                    agent,
                    pack: pack.name,
                    priority: pack.priority || 0,
 ...
 */
function mergeAgentBindings(packs) {
    const warnings = [];
    ...
    for (const pack of packs) {
        ...
        for (const [action, agentId] of Object.entries(bindings)) {
            ...
            if (!existing) {
                ...
                warnings.push(`agent binding for '${action}' overridden by '${pack.name}' (was '${existing.binding.provided_by}')`);
                bindingMap.set(action, {
                    binding: { action, agent_id: agentId, provided_by: pack.name },
                    priority: pack.priority || 0,
                });
    ...
    for (const pack of packs) {
        for (const meeting of pack.provides?.meetings || []) {
            ...
            if (!existing) {
                ...
                warnings.push(`meeting '${meeting.id}' overridden by higher-priority pack '${pack.name}' (was '${existing.pack}')`);
                meetingMap.set(meeting.id, {
                    meeting,
                    pack: pack.name,
                    priority: pack.priority || 0,

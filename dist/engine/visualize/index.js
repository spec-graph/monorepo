 ...
 */
async function generateDot(projectRoot, graph) {
    const lines = [];
            ...
            for (const t of transitions) {
                if (typeof t === "string" && t.includes("→")) {
                    ...
                    const gateLabel = gate.require_artifacts?.length
                        ...
                        : gate.id;
                    lines.push(`  "${from}" -> "${to}" [label="${gateLabel}", color="#F44336", style=bold, constraint=false];`);
                }
            }
        }
        lines.push("");
 ...
 */
async function generateMermaid(projectRoot, graph) {
    const lines = [];
 ...
 */
function generateSummary(graph) {
    const stages = graph.pipeline_skeleton?.stages || [];
    return {
        stages,
        artifact_count: graph.artifacts?.length || 0,
        artifact_kinds: Object.keys((graph.artifacts || []).reduce((acc, a) => {

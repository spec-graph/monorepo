...
exports.builtinChecks = void 0;
exports.isBuiltinCheck = isBuiltinCheck;
exports.extractBuiltinName = extractBuiltinName;
exports.runBuiltinCheck = runBuiltinCheck;
...
const scope_lock_1 = require("../isolation/scope-lock");
const index_1 = require("../trace/index");
// -- Registry --
exports.builtinChecks = {
...
function isBuiltinCheck(command) {
    const name = extractBuiltinName(command);
    return name !== null && name in exports.builtinChecks;
}
function extractBuiltinName(command) {
    const match = command.trim().match(/^<([^>]+)>$/);
    return match ? match[1] : null;
}
async function runBuiltinCheck(name, ctx) {
    const fn = exports.builtinChecks[name];
    if (!fn) {
        return {
            passed: false,
            exit_code: 127,
...
async function walkDir(dir, cb) {
    ...
    for (const e of entries) {
        const fp = node_path_1.default.join(dir, e.name);
        if (e.isDirectory()) {
...
async function readUtf(fp) {
    return promises_1.default.readFile(fp, "utf-8");
}
function rel(root, fp) {
    return node_path_1.default.relative(root, fp);
}
// -- clarify-scan --
const PATTERNS = [
...
async function runClarifyScan(ctx) {
    const roots = [
        node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts"),
        node_path_1.default.join(ctx.projectRoot, "_wdf_output"),
    ];
    const files = [];
    ...
    const issues = [];
    let totalAmbiguity = 0, placeholders = 0, nonMeasurable = 0;
        ...
        for (let i = 0; i < lines.length; i++) {
            for (const { re, w, cat } of PATTERNS) {
                ...
                while ((m = r.exec(lines[i])) !== null) {
                    ...
                    totalAmbiguity += w;
                    if (PLACEHOLDER_CATS.has(cat))
                        placeholders++;
                    if (cat === "non-measurable")
                        nonMeasurable++;
                }
            }
        }
    }
    const th = ctx.graph.checks.find((c) => c.id === "clarify-scan")
        ?.threshold;
    const constitution = await (0, constitution_1.loadConstitution)(ctx.projectRoot);
    ...
    const passed = totalAmbiguity <= maxA && placeholders <= maxP && nonMeasurable <= maxNM;
    const out = [
    ...
    if (!passed && issues.length > 0) {
        ...
        if (issues.length > 20)
            out.push(`  ... +${issues.length - 20} more`);
    }
    return {
        passed,
...
async function runStorySlicing(ctx) {
    const storyFiles = [];
    for (const root of [
        node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts/plan"),
        node_path_1.default.join(ctx.projectRoot, "_wdf_output/stories"),
    ]) {
        try {
            const entries = await promises_1.default.readdir(root);
            storyFiles.push(...entries
                .filter((e) => e.endsWith(".md"))
                .map((e) => node_path_1.default.join(root, e)));
        }
        catch { }
    }
    ...
    const th = ctx.graph.checks.find((c) => c.id === "story-slicing")
        ?.threshold;
    ...
    for (const r of results) {
        ...
        if (r.missing.length > 0)
            out.push(`    missing: ${r.missing.join(", ")}`);
    }
    return {
        passed: allPassed,
...
async function runAcTestBinding(ctx) {
    const storyFiles = [];
    for (const root of [
        node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts/plan"),
        node_path_1.default.join(ctx.projectRoot, "_wdf_output/stories"),
    ...
    for (const fp of storyFiles) {
        ...
        if (acItems > 0 && bindings < acItems && !hasSection)
            allPassed = false;
        results.push({
            file: rel(ctx.projectRoot, fp),
            ac: acItems,
    ...
    for (const r of results) {
        if (r.ac === 0)
            out.push(`  ~ ${r.file}: no ACs`);
        else
            out.push(`  ${r.bound >= r.ac ? "✓" : "✗"} ${r.file}: ${r.bound}/${r.ac} bound`);
    }
    return {
        passed: allPassed,
        exit_code: allPassed ? 0 : 1,
...
// -- architecture-review --
async function runArchitectureReview(ctx) {
    const archFiles = [];
    for (const root of [
        node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts/design"),
        node_path_1.default.join(ctx.projectRoot, "_wdf_output/architecture"),
        node_path_1.default.join(ctx.projectRoot, "docs"),
    ]) {
        await walkDir(root, async (fp) => {
            const n = node_path_1.default.basename(fp).toLowerCase();
    ...
    for (const r of results) {
        out.push(`  ${r.missing.length === 0 ? "✓" : "✗"} ${r.file}: [${r.found.join(", ")}]`);
        if (r.missing.length > 0)
            out.push(`    MISSING: ${r.missing.join(", ")}`);
    }
    return {
...
// -- data-model-validate --
async function runDataModelValidate(ctx) {
    const files = [];
    for (const root of [
        node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts"),
        node_path_1.default.join(ctx.projectRoot, "_wdf_output"),
    ...
    if (files.length === 0)
        return dataModelEmptyResult(ctx);
    const issues = [];
    for (const fp of files) {
        const content = await readUtf(fp);
        ...
        if (issue)
            issues.push(`${rel(ctx.projectRoot, fp)}: ${issue}`);
    }
...
function dataModelEmptyResult(ctx) {
...
function validateSchemaFile(content, ext) {
    if (ext === ".yaml" || ext === ".yml") {
        if (!/^(entities|tables|models|schema)/m.test(content))
            return "missing entities/tables key";
    }
    else if (ext === ".md") {
        if (!/(?:entity|table|model|collection)\s*[:|]/i.test(content) &&
            !/(?:实体|表|模型)/.test(content))
            return "no entity definitions";
    }
    else if (ext === ".prisma") {
        if (!/model\s+\w+/.test(content))
            return "no Prisma models";
    }
    return null;
}
// -- schema-drift-scan --
async function runSchemaDrift(ctx) {
    const driftPath = node_path_1.default.join(ctx.projectRoot, ".spec-graph/schema-drift.json");
    const files = [];
...
async function computeFileHashes(projectRoot, files) {
    const hashes = {};
    for (const fp of files) {
        const content = await readUtf(fp);
        hashes[rel(projectRoot, fp)] = node_crypto_1.default
            .createHash("sha256")
            .update(content)
            .digest("hex")
            .slice(0, 16);
...
async function loadPrevHashes(driftPath) {
    try {
...
async function saveHashes(driftPath, hashes) {
    await promises_1.default.mkdir(node_path_1.default.dirname(driftPath), { recursive: true });
    await promises_1.default.writeFile(driftPath, JSON.stringify({ hashes, scanned_at: new Date().toISOString() }, null, 2));
}
function computeDrift(prev, curr) {
    const drifted = [], added = [], removed = [];
...
function renderDriftResult(count, diff, prev, curr) {
    ...
    if (diff.drifted.length > 0) {
        ...
        for (const f of diff.drifted)
            out.push(`    ${f}: ${prev[f]} → ${curr[f]}`);
    }
    if (diff.added.length > 0)
        out.push(`  + new: ${diff.added.join(", ")}`);
    if (diff.removed.length > 0)
        out.push(`  - removed: ${diff.removed.join(", ")}`);
    return out.join("\n");
}
// -- complexity-scan --
async function runComplexityScan(ctx) {
...
async function getComplexityThreshold(ctx) {
    const th = ctx.graph.checks.find((c) => c.id === "complexity-budget")
        ?.threshold;
    const constitution = await (0, constitution_1.loadConstitution)(ctx.projectRoot);
...
async function scanFileComplexity(projectRoot, fp, maxCC) {
    const lines = (await readUtf(fp)).split("\n");
    const violations = [];
    let inFn = false, fnName = "", fnLine = 0, cc = 1, depth = 0, fnDepth = 0;
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const m = line.match(/(?:async\s+)?(?:function\s+(\w+)|(?:const|let)\s+(\w+)\s*=\s*(?:async\s+)?\()/);
        if (m && !inFn) {
            inFn = true;
            fnName = m[1] || m[2] || "anon";
            fnLine = i + 1;
        ...
        if (inFn) {
            if (/\b(if|else\s+if|catch|case)\b/.test(line))
                cc++;
            if (/\b(for|while|do)\b\s*[\({]/.test(line))
                cc++;
...
function renderComplexityResult(fileCount, violations, maxCC) {
...
async function runConstitutionValidate(ctx) {
    const constitution = await (0, constitution_1.loadConstitution)(ctx.projectRoot);
    if (!constitution) {
        return {
            passed: false,
            exit_code: 1,
            stdout: "constitution-validate: no constitution found. Run `spec-graph constitution init`.",
            stderr: "",
    ...
    if (graphDrifts.length > 0) {
        out.push(`  ⚠ ${graphDrifts.length} pack drift(s) (constitution wins at runtime):`);
        for (const d of graphDrifts)
            out.push(`    • ${d}`);
    }
    return {
...
function validateConstitutionSchema(c, errors, warnings) {
    if (!c.version)
...
async function detectConstitutionPackDrift(ctx) {
    const constitution = await (0, constitution_1.loadConstitution)(ctx.projectRoot);
    if (!constitution)
        return [];
    ...
    const constT = constitution.quality.thresholds;
    const drifts = [];
    const complexityCheck = checks.find((c) => c.id === "complexity-budget");
    const packCC = complexityCheck?.threshold?.cyclomatic;
    if (constT.cyclomatic_complexity !== undefined &&
        packCC !== undefined &&
        constT.cyclomatic_complexity !== packCC) {
        drifts.push(`complexity-budget: pack=${packCC}, constitution=${constT.cyclomatic_complexity}`);
    }
    const clarifyCheck = checks.find((c) => c.id === "clarify-scan");
    const packAmb = clarifyCheck?.threshold?.ambiguity;
    if (constT.ambiguity_score !== undefined &&
        packAmb !== undefined &&
        constT.ambiguity_score !== packAmb) {
        drifts.push(`clarify-scan: pack ambiguity=${packAmb}, constitution=${constT.ambiguity_score}`);
    }
    return drifts;
}
// -- contract-drift-scan --
...
async function runContractDriftScan(ctx) {
    const dir = node_path_1.default.join(ctx.projectRoot, ".spec-graph", "contracts");
    let entries = [];
    try {
        ...
        return {
            passed: true,
            exit_code: 0,
            stdout: "contract-drift-scan: no contract registry yet (nothing to drift)",
            stderr: "",
    ...
    for (const d of drifted) {
        out.push(`  • ${d.contract} / ${d.consumer}: ${d.status} (bound ${d.bound} ≠ current ${d.current})`);
    }
    return {
...
async function runScopeLockValidate(ctx) {
    const scopeDir = node_path_1.default.join(ctx.projectRoot, ".spec-graph", "isolation");
    let files = [];
    try {
        const all = await promises_1.default.readdir(scopeDir);
        ...
        return {
            passed: true,
            exit_code: 0,
            stdout: "scope-lock-validate: no scope locks registered (passing vacuously)",
            stderr: "",
    ...
    const wtRegPath = node_path_1.default.join(scopeDir, "worktrees.yaml");
    const wtReg = await (0, yaml_1.tryReadYaml)(wtRegPath);
    const units = {};
    if (wtReg?.units) {
        for (const [id, u] of Object.entries(wtReg.units)) {
    ...
    for (const f of files) {
        const lock = await (0, yaml_1.tryReadYaml)(node_path_1.default.join(scopeDir, f));
        if (!lock)
            continue;
        ...
        const result = (0, scope_lock_1.validateScopeLock)(changedFiles, lock);
        results.push({
            unit: unitId,
...
async function gitDiffFiles(cwd, since) {
    return new Promise((resolve) => {
        const proc = (0, node_child_process_1.spawn)("git", ["diff", "--name-only", since, "HEAD"], {
            cwd,
...
async function runCloneDetection(ctx) {
    const srcDir = node_path_1.default.join(ctx.projectRoot, "src");
    const files = await collectSourceFiles(ctx.projectRoot);
    if (files.length === 0) {
        return {
            passed: true,
...
function getCloneThreshold(ctx) {
    const check = ctx.graph.checks.find((c) => c.id === "clone-detection");
...
function extractFunctions(content) {
    ...
    while (i < lines.length) {
        const line = lines[i];
        const m = line.match(/^\s*(?:export\s+)?(?:async\s+)?(?:function\s+(\w+)|(?:const|let)\s+(\w+)\s*=\s*(?:async\s+)?\()/);
        if (!m) {
            i++;
            continue;
        }
...
function hashTokens(source) {
    ...
    s = s.replace(/\b([a-zA-Z_$][a-zA-Z0-9_$]*)\b/g, (m) => keywords.has(m) ? m : "id");
    // Collapse whitespace
...
async function runReuseScan(ctx) {
    const files = await collectSourceFiles(ctx.projectRoot);
    if (files.length === 0) {
        return {
            passed: true,
    ...
    for (const fp of files) {
        ...
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const m = lines[i].match(/^\s*export\s+(?:async\s+)?(?:function|const|let|class)\s+(\w+)/);
            if (m) {
    ...
    let traceIndex = null;
    try {
        traceIndex = await (0, index_1.buildTraceIndex)(ctx.projectRoot, ctx.graph);
        satisfiesEdges = traceIndex.edges.filter((e) => e.relation === "satisfies").length;
    }
    catch {
    ...
    const contractsDir = node_path_1.default.join(ctx.projectRoot, ".spec-graph", "contracts");
    let contractCount = 0;
    try {
    ...
    if (orphans.length > 0) {
        ...
        if (orphans.length > 15)
            out.push(`    ... +${orphans.length - 15} more`);
...
async function runAcceptanceLayerAudit(ctx) {
    ...
    for (const layer of expected) {
        ...
        if (def.required && checks.length === 0) {
            ...
            warnings.push(`layer '${layer}' is required but has no checks — cannot verify acceptance`);
        }
    ...
    for (const layerName of Object.keys(layers)) {
        if (!expected.includes(layerName)) {
            warnings.push(`layer '${layerName}' is not a standard acceptance layer (expected: ${expected.join(", ")})`);
        }
    }
    ...
    for (const r of results) {
        ...
        out.push(`  ${icon} ${r.layer} [${reqLabel}]: ${r.check_count} check(s) — ${r.status}`);
    }
    if (warnings.length > 0) {
        out.push("  warnings:");
        for (const w of warnings)
...
async function runSharedContractAudit(ctx) {
    ...
    if (opportunities.length === 0) {
        out.push("  ✓ no orphan-consumed contracts (all consumed contracts have producers)");
    }
    else {
        out.push("  suggestions for human review (do NOT auto-wire — premature abstraction is worse):");
        for (const opp of opportunities) {
            ...
            out.push(`    • ${opp.contract} ← ${opp.consumers.join(", ")} (${opp.consumer_count} consumers, 0 producers)`);
...
async function runCommandSafetyValidate(ctx) {
    const constitution = await (0, constitution_1.loadConstitution)(ctx.projectRoot);
    const whitelist = constitution?.security?.command_whitelist || FALLBACK_COMMAND_WHITELIST;
    const forbidden = constitution?.security?.forbidden_patterns || FALLBACK_FORBIDDEN_PATTERNS;
    const checks = ctx.graph.checks || [];
    const violations = [];
    for (const check of checks) {
        ...
        if (extractBuiltinName(command))
            continue;
        // Whitelist prefix match
        const matchesWhitelist = whitelist.some((prefix) => command.startsWith(prefix));
        if (!matchesWhitelist) {
            violations.push({
                check_id: check.id,
                command,
...
async function runArticlesValidate(ctx) {
    const constitution = await (0, constitution_1.loadConstitution)(ctx.projectRoot);
    ...
    const artifactsDir = node_path_1.default.join(ctx.projectRoot, ".spec-graph", "artifacts");
    const violations = [];
    for (const article of articles) {
        const rule = article.rule;
        if (rule.type === "required_section" || rule.type === "min_length") {
            // File-based rules: check artifact content
            const artifactFile = node_path_1.default.join(artifactsDir, `${rule.artifact_kind}.md`);
            ...
            if (rule.type === "required_section") {
                ...
                const sectionRe = new RegExp(`^#{1,6}.*${escapeRegExp(rule.section)}.*$`, "im");
            ...
            const fromEntries = Object.values(ctx.state.artifacts || {}).filter((a) => a.id.startsWith(rule.from_kind) || a.id === rule.from_kind);
            const hasCompletedFrom = fromEntries.some((a) => a.status === "completed");
            if (hasCompletedFrom) {
                const toEntries = Object.values(ctx.state.artifacts || {}).filter((a) => a.id.startsWith(rule.to_kind) || a.id === rule.to_kind);
...
async function runBoundedContextAudit(ctx) {
    const tracks = ctx.graph.tracks || [];
    if (tracks.length < 2) {
        return {
            passed: true,
            exit_code: 0,
            stdout: "bounded-context-audit: fewer than 2 tracks (passing vacuously — no cross-boundary possible)",
            stderr: "",
            details: {
    ...
    for (const track of tracks) {
        if (track.scope) {
            scopeDirs.push({
                scope: track.scope,
                dir: node_path_1.default.join(srcDir, track.scope),
                trackId: track.id,
            });
        }
    }
    ...
    for (const scopeEntry of scopeDirs) {
        ...
        for (const fp of files) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                const importMatch = line.match(/(?:import\s+.*?from\s+['"]([^'"]+)['"]|require\s*\(\s*['"]([^'"]+)['"]\s*\))/);
                if (!importMatch)
                    continue;
                ...
                const resolvedDir = node_path_1.default.dirname(fp);
                const resolvedImport = node_path_1.default.resolve(resolvedDir, importPath);
                // Check if the resolved import is in a different scope
                for (const otherScope of scopeDirs) {
                    if (otherScope.scope === scopeEntry.scope)
                        continue;
    ...
    if (violations.length === 0) {
        ...
        for (const v of violations.slice(0, 20)) {
            out.push(`    • ${v.file}:${v.line}: ${v.from_scope} → ${v.to_scope} (${v.import_path})`);
        }
...
async function runAggregateInvariantCheck(ctx) {
    const artifactPath = node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts/design/aggregates.md");
    ...
    if (!stateEntry || stateEntry.status !== "completed") {
        return {
            passed: true,
            exit_code: 0,
            stdout: "aggregate-invariant-check: design/aggregates not completed (skipping)",
            stderr: "",
            details: {
                aggregates_found: 0,
    ...
    try {
        ...
        return {
            passed: false,
            exit_code: 1,
            stdout: "aggregate-invariant-check: artifact file not found: design/aggregates.md",
    ...
    const aggregateHeaders = content.match(/^#{2,4}\s+.*?(聚合|Aggregate)[:\s].*$/gim) || [];
    const aggregates = aggregateHeaders.map((h) => h.replace(/^#{2,4}\s+/, "").trim());
...
async function runContextMapConsistency(ctx) {
    const artifactPath = node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts/design/context-map.md");
    const stateEntry = ctx.state.artifacts?.["design/context-map"];
    if (!stateEntry || stateEntry.status !== "completed") {
        return {
            passed: true,
            exit_code: 0,
            stdout: "context-map-consistency: design/context-map not completed (skipping)",
            stderr: "",
    ...
    const relationPattern = new RegExp(`\\|\\s*([^|]+)\\s*\\|\\s*([^|]+)\\s*\\|\\s*(${relationTypes.join("|")})`, "gi");
    ...
    while ((match = relationPattern.exec(content)) !== null) {
        ...
        if (upstream.includes("---") || downstream.includes("---"))
            continue;
        if (upstream.toLowerCase().includes("upstream") ||
        ...
        for (const f of files.filter((f) => f.endsWith(".yaml") || f.endsWith(".yml"))) {
            ...
            if (entry) {
                contracts.push({
                    ...
                    consumers: (entry.consumers || []).map((c) => ({
                        consumer: c.consumer || "",
    ...
    for (const rel of declaredRelations) {
        const hasContract = contracts.some((c) => (c.producer === rel.upstream || c.producer.includes(rel.upstream)) &&
            c.consumers.some((consumer) => consumer.consumer === rel.downstream ||
                consumer.consumer.includes(rel.downstream)));
        if (!hasContract) {
            warnings.push(`declared ${rel.upstream} → ${rel.downstream} (${rel.type}) but no matching contract binding found`);
        }
    }
    const out = [
...
async function runDomainEventCoverage(ctx) {
    const artifactPath = node_path_1.default.join(ctx.projectRoot, ".spec-graph/artifacts/design/domain-events.md");
    const stateEntry = ctx.state.artifacts?.["design/domain-events"];
    if (!stateEntry || stateEntry.status !== "completed") {
        return {
            passed: true,
            exit_code: 0,
            stdout: "domain-event-coverage: design/domain-events not completed (skipping)",
            stderr: "",
    ...
    const tableRows = content.match(/^\|\s+([A-Z][A-Za-z]+(?:Event|Occurred|Created|Updated|Deleted|Completed)?)\s+\|/gm) || [];
    ...
    // From headings
    const headingEvents = content.match(/^#{2,4}\s+([A-Z][A-Za-z]+Event)/gm) || [];

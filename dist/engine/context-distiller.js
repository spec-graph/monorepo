...
exports.distillContext = distillContext;
/**
 ...
 */
function distillContext(actionId, graph, traceIndex, state, maxHops = 3) {
    const visited = new Set();
    const result = [];
    const queue = [{ id: actionId, hops: 0 }];
    while (queue.length > 0) {
        ...
        if (artifactDecl) {
            for (const otherDecl of graph.artifacts || []) {
                if (otherDecl.id === id)
                    continue;
                if (visited.has(otherDecl.id))
                    continue;
                // Check if there's a producer/consumer relationship
                if (artifactDecl.default_consumers?.includes(otherDecl.id) ||
...
//# sourceMappingURL=context-distiller.js.map
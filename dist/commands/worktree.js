...
exports.worktreeCommand = worktreeCommand;
const chalk_1 = __importDefault(require("chalk"));
const worktree_1 = require("../engine/isolation/worktree");
async function worktreeCommand(projectRoot, options) {
    const sub = options.subcommand || "list";
    const wm = new worktree_1.WorktreeManager({ projectRoot });
    try {
        switch (sub) {
            case "create":
            ...
            default:
                ...
                console.log("Available: create, list, remove, merge, status, self-verify, submit, accept, reject");
                process.exit(1);
        }
    }
    catch (e) {
 ...
 */
async function transitionStateCmd(wm, options, newState) {
    if (!options.unitId) {
        console.log(chalk_1.default.red("✗ --unit-id is required"));
        process.exit(1);
        return;
    }
    const unit = await wm.get(options.unitId);
    if (!unit) {
        console.log(chalk_1.default.red(`✗ Isolation unit not found: ${options.unitId}`));
        process.exit(1);
        return;
    }
    ...
    if (!allowedFrom.includes(unit.status)) {
        console.log(chalk_1.default.red(`✗ Invalid transition: ${unit.status} → ${newState}. Valid from: ${allowedFrom.join(", ")}`));
        process.exit(1);
        return;
    }
    const now = new Date().toISOString();
    unit.status = newState;
    if (newState === "self_verified")
        unit.self_verified_at = now;
    ...
    if (options.json) {
        ...
        console.log(chalk_1.default.green(`✓ Unit ${unit.id} transitioned to '${newState}'`));
        if (newState === "rejected" && options.reason) {
            console.log(chalk_1.default.gray(`  Reason: ${options.reason}`));
        }
        if (options.reviewedBy) {
            console.log(chalk_1.default.gray(`  Reviewed by: ${options.reviewedBy}`));
        }
    }
}
async function createCmd(wm, opts) {
    if (!opts.unitId) {
        console.log(chalk_1.default.red("✗ Unit ID required. Usage: spec-graph worktree create <id> --track <track>"));
        process.exit(1);
        return;
    }
    if (!opts.track) {
        console.log(chalk_1.default.red("✗ Track required. Usage: spec-graph worktree create <id> --track <track>"));
        process.exit(1);
        return;
    }
...
async function listCmd(wm, opts) {
    const units = await wm.list();
    if (units.length === 0) {
        console.log(chalk_1.default.yellow("\nNo isolation units."));
        console.log(chalk_1.default.gray("  Run `spec-graph worktree create <id> --track <track>` to create one.\n"));
        return;
    }
    if (opts.json) {
    ...
    for (const u of units) {
        ...
        console.log(`  • ${u.id} [${statusColor(u.status)}] track=${u.track} branch=${u.branch}`);
    }
    console.log("");
}
async function removeCmd(wm, opts) {
    if (!opts.unitId) {
        console.log(chalk_1.default.red("✗ Unit ID required. Usage: spec-graph worktree remove <id>"));
        process.exit(1);
        return;
    }
...
async function mergeCmd(wm, opts) {
    if (!opts.unitId) {
        console.log(chalk_1.default.red("✗ Unit ID required. Usage: spec-graph worktree merge <id> --to <branch>"));
        process.exit(1);
        return;
    }
    ...
    if (result.success) {
        ...
        if (result.mergedCommit) {
            console.log(chalk_1.default.gray(`  Commit: ${result.mergedCommit.substring(0, 8)}\n`));
        }
    }
    else {
        ...
        if (result.conflicts.length > 0) {
            console.log(chalk_1.default.yellow(`  Conflicts (${result.conflicts.length}):`));
...
async function statusCmd(wm, opts) {
    if (!opts.unitId) {
    ...
    if (!unit) {
        console.log(chalk_1.default.red(`✗ Unit not found: ${opts.unitId}`));
        process.exit(1);
        return;
    }
    if (opts.json) {
    ...
    if (unit.merged_at)
        console.log(`  Merged:  ${unit.merged_at}`);
    console.log("");
}

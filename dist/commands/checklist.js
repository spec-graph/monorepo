...
async function checklistCommand(projectRoot, storyId, options) {
    try {
        ...
        if (!storyArtifact) {
            console.log(chalk_1.default.red(`✗ Story not found: ${storyId}`));
            console.log(chalk_1.default.gray("Available stories:"));
            const stories = Object.keys(state.artifacts).filter((id) => id.startsWith("plan/story/"));
        ...
        if (storyArtifact.status !== "pending" &&
            storyArtifact.status !== "in_progress") {
            console.log(chalk_1.default.yellow(`⚠ Story ${storyId} is already ${storyArtifact.status}. Checklist is for pre-implementation validation.`));
        }
        // Generate checklist
        const checklist = await generateChecklist(storyId, storyArtifact, graph, state, traceIndex);
        // Write to file
        const checklistsDir = node_path_1.default.join(specGraphDir, "checklists");
        await promises_1.default.mkdir(checklistsDir, { recursive: true });
        const checklistPath = node_path_1.default.join(checklistsDir, `${storyId.replace(/\//g, "_")}.md`);
        await promises_1.default.writeFile(checklistPath, checklist.content, "utf-8");
        if (options.json) {
        ...
        console.log(chalk_1.default.green(`✓ Checklist generated: ${checklistPath}`));
        console.log("");
        console.log(chalk_1.default.bold("Mechanical Checks (automated):"));
        for (const check of checklist.mechanicalChecks) {
            const status = check.passed ? chalk_1.default.green("✓") : chalk_1.default.red("✗");
        ...
        console.log("");
        console.log(chalk_1.default.bold("Soft Checks (manual review):"));
        ...
        console.log("");
        console.log(chalk_1.default.gray("Edit the checklist file to mark soft checks as complete."));
    }
    catch (e) {
...
async function generateChecklist(storyId, storyArtifact, graph, state, traceIndex) {
    const mechanicalChecks = [];
    const softChecks = [];
    // Load document content for content-based checks
    let documentContent;
    try {
        const artifactsDir = node_path_1.default.join(process.cwd(), '.spec-graph', 'artifacts');
        const docPath = node_path_1.default.join(artifactsDir, storyId.replace(/\//g, '_') + '.md');
        // Also try the actual story path
        const altPath = node_path_1.default.join(artifactsDir, 'story', storyId.split('/').pop() + '.md');
        try {
            documentContent = await promises_1.default.readFile(docPath, 'utf-8');
        }
        catch {
            try {
                documentContent = await promises_1.default.readFile(altPath, 'utf-8');
            }
            catch {
    ...
    const hasReqResolution = checkReqResolution(storyId, graph, state, traceIndex);
    mechanicalChecks.push({
...
function checkReqMapping(storyId, graph, state, traceIndex) {
    // Check if story has 'derives' edges to any requirement artifacts in the trace index
    if (!traceIndex || !traceIndex.edges) {
        return { passed: false };
    }
    const derivesEdges = traceIndex.edges.filter((e) => e.from === storyId && e.relation === "derives");
    // Check if any of the 'to' nodes are requirement artifacts
    const hasRequirementTarget = derivesEdges.some((e) => {
...
function checkAtomicScope(storyArtifact, documentContent) {
    if (!documentContent)
        return { passed: true };
...
function checkACCount(storyArtifact, documentContent) {
    if (!documentContent)
        return { passed: true };
...
function checkPathSafety(storyArtifact, documentContent) {
    if (!documentContent)
        return { passed: true };
...
function checkReqResolution(storyId, graph, state, traceIndex) {
    ...
    const allResolved = reqIds.every((reqId) => {
 ...
 */
function detectAmbiguousAdjectives(documentContent) {
    if (!documentContent)
        return null;
    const ambiguous = [
...
function generateMarkdown(storyId, mechanicalChecks, softChecks) {
    const lines = [];

...
async function doctorCommand(projectRoot, options) {
    ...
    // ── Category: compose ──
    let graph = await checkCompose(specGraphDir, checks);
    // ── Category: state ──
    ...
    const statusIcon = ok ? chalk_1.default.green("✓") : chalk_1.default.red("✗");
    console.log(`  Status: ${statusIcon} ${errors} error(s), ${warnings} warning(s)\n`);
    // ── Category groups ──
    const categories = [
        "init",
        "compose",
    ...
    for (const cat of categories) {
        ...
        if (catChecks.length === 0)
            continue;
        console.log(chalk_1.default.bold(`  ${catLabel(cat)}`));
        for (const c of catChecks) {
            const icon = c.severity === "ok"
            ...
            if (c.detail) {
                console.log(chalk_1.default.gray(`    ${c.detail}`));
            }
        }
        console.log("");
    }
    if (!ok) {
        console.log(chalk_1.default.yellow("  Run `spec-graph doctor --fix` to auto-fix recoverable issues.\n"));
        process.exitCode = 1;
    }
    else {
...
async function checkInit(projectRoot, specGraphDir, checks) {
    // Directory exists
    try {
    ...
    try {
        ...
        const profile = await (0, yaml_1.readYaml)(profilePath);
    ...
    // permissions.yaml
    const permPath = node_path_1.default.join(specGraphDir, "permissions.yaml");
    try {
        await promises_1.default.access(permPath);
        checks.push({
            id: "init-permissions",
            category: "init",
            severity: "ok",
...
async function checkCompose(specGraphDir, checks) {
    const graphPath = node_path_1.default.join(specGraphDir, "graph.yaml");
    try {
        await promises_1.default.access(graphPath);
    }
    catch {
        checks.push({
            id: "compose-graph",
    ...
    if (!graph.pipeline_skeleton)
        issues.push("missing pipeline_skeleton");
    ...
    if (!graph.artifacts?.length &&
        ...
        checks.push({
            ...
            detail: "The compose step may not have matched any packs. Check your profile.",
        });
    }
    return graph;
}
async function checkState(specGraphDir, graph, checks) {
    const statePath = node_path_1.default.join(specGraphDir, "machine-state.yaml");
    try {
        await promises_1.default.access(statePath);
    }
    catch {
        checks.push({
            id: "state-file",
    ...
    let state;
    try {
        state = await (0, yaml_1.readYaml)(statePath);
    }
    catch (e) {
        checks.push({
            id: "state-parse",
...
async function checkPermissions(specGraphDir, checks) {
    const permPath = node_path_1.default.join(specGraphDir, "permissions.yaml");
    try {
        await promises_1.default.access(permPath);
    }
    ...
    let perms;
    try {
        perms = await (0, yaml_1.readYaml)(permPath);
    }
    catch (e) {
        checks.push({
            id: "perms-parse",
...
async function checkTraces(specGraphDir, graph, checks) {
    const tracesDir = node_path_1.default.join(specGraphDir, "traces");
    let entries;
    try {
        entries = (await promises_1.default.readdir(tracesDir)).filter((e) => e.endsWith(".yaml"));
    }
    catch {
    ...
    for (const entry of entries) {
        try {
            const trace = await (0, yaml_1.readYaml)(node_path_1.default.join(tracesDir, entry));
    ...
    if (placeholderCount > 0)
        parts.push(`${placeholderCount} placeholder(s)`);
    checks.push({
        id: "traces-summary",
        category: "traces",
...
async function checkConsistency(specGraphDir, graph, checks) {
    const statePath = node_path_1.default.join(specGraphDir, "machine-state.yaml");
    let state;
    try {
        state = await (0, yaml_1.readYaml)(statePath);
    ...
    const orphanArtifacts = stateArtifactIds.filter((id) => !graphArtifactIds.has(id));
    ...
    const missingArtifacts = (graph.artifacts || []).filter((a) => !stateArtifactIds.includes(a.id));
    ...
    const missingChecks = (graph.checks || []).filter((c) => !stateCheckIds.includes(c.id));
...
async function checkFeatures(projectRoot, specGraphDir, graph, checks) {
    // Hooks
    const hooksPath = node_path_1.default.join(specGraphDir, "hooks.yaml");
    try {
        await promises_1.default.access(hooksPath);
        checks.push({
            id: "feat-hooks",
            category: "features",
            severity: "ok",
    ...
    if (graph) {
        const checksWithTouchfiles = (graph.checks || []).filter((c) => c.touchfiles && c.touchfiles.length > 0);
        const periodicChecks = (graph.checks || []).filter((c) => c.tier === "periodic");
    ...
    // Safety-net snapshot
    const snapshotPath = node_path_1.default.join(specGraphDir, "safety-net-snapshot.yaml");
    try {
        await promises_1.default.access(snapshotPath);
        checks.push({
            id: "feat-safety-net",
            category: "features",
            severity: "ok",
    ...
    // Constitution
    const constPath = node_path_1.default.join(specGraphDir, "constitution.yaml");
    try {
        await promises_1.default.access(constPath);
        checks.push({
    ...
    // Scope locks
    const isolationDir = node_path_1.default.join(specGraphDir, "isolation");
    try {
        const lockFiles = await promises_1.default.readdir(isolationDir);
...
async function autoFix(projectRoot, specGraphDir, checks) {
    let fixed = 0;
    ...
    if (checks.some((c) => c.id === "state-file" && c.severity === "error")) {
        try {
            const graph = await (0, yaml_1.readYaml)(node_path_1.default.join(specGraphDir, "graph.yaml")).catch(() => null);
            if (graph) {
                ...
                await (0, yaml_1.writeYaml)(node_path_1.default.join(specGraphDir, "machine-state.yaml"), state);
                fixed++;
                // Update the check
    ...
    if (checks.some((c) => c.id === "init-permissions" && c.severity === "warn")) {
        try {
            const config = (0, index_1.getPreset)("semi-auto");
